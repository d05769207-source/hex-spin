...
/**
 * TEST: Verify Tie-Breaker Logic
 * Creates 2 users with SAME score but DIFFERENT times.
 * User A: 1000 coins (Time T)
 * User B: 1000 coins (Time T + 1 min)
 * Expectation: User A should be ranked HIGHER than User B
 */
export const verifyTieBreaker = async (): Promise<string> => {
    try {
        console.log('🧪 Starting Tie-Breaker Test...');
        const weekId = getCurrentWeekId();

        // 1. Create User A (Early Bird)
        const userA_ID = 'test_user_A';
        const userA_Ref = doc(db, 'weeklyLeaderboard', `${userA_ID}_${weekId}`);
        await setDoc(userA_Ref, {
            userId: userA_ID,
        ...
        // 2. Create User B (Late Arrival)
        const userB_ID = 'test_user_B';
        const userB_Ref = doc(db, 'weeklyLeaderboard', `${userB_ID}_${weekId}`);
        await setDoc(userB_Ref, {
            userId: userB_ID,
        ...
        console.log('✅ Created Test Users: A (1 min ago) and B (Now) with 1000 coins.');

        // 3. Fetch Leaderboard to check order (Run query directly to catch Index Errors)
        // const { getWeeklyLeaderboard } = await import('./leaderboardService');
        // const leaderboard = await getWeeklyLeaderboard(100, false);
        ...
        try {
            ...
            snapshot.forEach(doc => leaderboard.push({ userId: doc.data().userId, rank: leaderboard.length + 1 }));
        } catch (queryError: any) {
            console.error("Query Error:", queryError);
            if (queryError.message.includes("index")) {
                return "⚠️ MISSING INDEX ERROR:\n\nFirebase requires an index for this sort.\n\nOpen your Browser Console (F12) -> Console Tab.\nClick the link generated by Firebase to create the index automatically.";
            }
            throw queryError;
        }

        // Find ranks
        const rankA = leaderboard.find(u => u.userId === userA_ID)?.rank;
        const rankB = leaderboard.find(u => u.userId === userB_ID)?.rank;

        let result = `Test Results:\nUser A Rank: ${rankA}\nUser B Rank: ${rankB}\n`;

        if (rankA !== undefined && rankB !== undefined) {
            if (rankA < rankB) {
                result += "✅ SUCCESS: User A is ranked higher (better rank) than User B.";
            } else {
                result += "❌ FAILURE: User A should be ranked higher due to earlier time.";
            }
        } else {
            result += "⚠️ Could not find test users in leaderboard.";
        }
...
/**
 * CLEANUP: Remove Tie-Breaker Test Users
 */
export const clearTestUsers = async (): Promise<string> => {
    try {
        console.log('🧹 Cleaning up test users...');
        const weekId = getCurrentWeekId();

        await deleteDoc(doc(db, 'weeklyLeaderboard', `test_user_A_${weekId}`));
        await deleteDoc(doc(db, 'weeklyLeaderboard', `test_user_B_${weekId}`));

        return '✅ Test users removed successfully!';
    } catch (error) {
