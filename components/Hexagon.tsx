...
const Hexagon: React.FC<HexagonProps> = ({ item, isActive, isWon, debugIndex }) => {
  ...
  // Mobile: Compact (70px/60px/48px)
  // Desktop: Larger & Grand for 420px container (95px/82px/66px)
  if (item.isInner) {
    if (item.id === 'inner-left' || item.id === 'inner-right') {
      // TIER 1: LARGEST (Bundles) - Left & Right
      sizeClasses = 'w-[70px] h-[60px] md:w-[95px] md:h-[82px]'; 
      zIndex = 'z-30';
    } else if (item.id === 'inner-top' || item.id === 'inner-bottom') {
      // TIER 2: MEDIUM (Gun & Car) - Top & Bottom
      sizeClasses = 'w-[60px] h-[52px] md:w-[82px] md:h-[71px]';
      zIndex = 'z-20';
    }
  } else {
    // TIER 3: SMALLEST (Outer Items)
    sizeClasses = 'w-[48px] h-[42px] md:w-[66px] md:h-[58px]';
    zIndex = 'z-10';
  }
