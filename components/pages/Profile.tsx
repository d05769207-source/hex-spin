...
const Profile: React.FC<ProfileProps> = ({ onBack, coins, tokens, user, onLogout }) => {
  return (
    <div className="w-full max-w-md mx-auto h-full flex flex-col p-4 animate-in slide-in-from-right duration-300">
      ...
      {/* Recent Wins */}
      <div className="flex-1 bg-black/20 rounded-xl p-4 border border-white/5 mb-6 overflow-y-auto">
      ...
      {/* Action Buttons */}
      <div className="grid gap-2 mb-20 md:mb-0">
        ...
        {user && !user.isGuest && (
            <button 
                onClick={onLogout}
                className="flex items-center justify-center gap-2 p-4 mt-2 bg-red-900/30 rounded-lg text-red-400 font-bold hover:bg-red-900/50 transition-colors border border-red-500/20"
            >
                <LogOut size={18} />
                Logout
            </button>
